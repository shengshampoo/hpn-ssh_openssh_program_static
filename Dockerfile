...
# required HPN-SSH openssh
RUN apk add --no-cache \
  gcc make linux-headers musl-dev zlib-dev zlib-static \
  python3-dev openssl-dev openssl-libs-static cmake git ninja \
...
ENV XZ_OPT=-e9
COPY build-static-hpnssh-openssh.sh build-static-hpnssh-openssh.sh
RUN chmod +x ./build-static-hpnssh-openssh.sh
RUN bash ./build-static-hpnssh-openssh.sh  
